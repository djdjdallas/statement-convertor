/**
 * API Route: /api/quickbooks/mappings/categories
 *
 * Save or delete category mappings
 */
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const mappings = await getCategoryMappings(connection.id);
      ...
      const savedMappings = await saveCategoryMappings(connection.id, mappings);
    ...
    console.error('Error managing category mappings:', error);
    return res.status(500).json({
      error: 'Failed to manage category mappings',
